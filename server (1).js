...
// ── Met.no daily (7 giorni, cache 6 ore) ────────────────────────────────
// api.met.no — servizio meteorologico norvegese, gratuito, no key, no rate limit
const DAILY_CACHE = {};
const DAILY_TTL = 6 * 60 * 60 * 1000;

// Mappa WMO weather code da simbolo met.no
function metnoSymbolToWMO(symbol) {
  ...
  if (symbol.includes('rain') || symbol.includes('shower')) return 61;
  if (symbol.includes('fog'))            return 45;
  return 3;
}

async function fetchMetnoDaily(lat, lon) {
  const key = `${lat},${lon}`;
  const cached = DAILY_CACHE[key];
  if (cached && Date.now() - cached.ts < DAILY_TTL) return cached.data;

  const url = `https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=${lat}&lon=${lon}`;
  console.log(`[MET] Fetching met.no daily for ${lat},${lon}`);
  const r = await fetch(url, {
  ...
  // Aggrega per giorno: prendi il valore di mezzogiorno per codice meteo
  const series = raw.properties.timeseries;
  const byDay = {};
  for (const ts of series) {
    const dt = new Date(ts.time);
    const dateStr = ts.time.slice(0, 10);
    const hour = dt.getUTCHours();
    const instant = ts.data.instant.details;
    const next1h = ts.data.next_1_hours || ts.data.next_6_hours;
    const symbol = next1h?.summary?.symbol_code || '';
    if (!byDay[dateStr]) byDay[dateStr] = { temps: [], symbols: [], precip: 0 };
    byDay[dateStr].temps.push(instant.air_temperature ?? 0);
    if (symbol) byDay[dateStr].symbols.push(symbol);
    // Usa precipitazioni dalle 6h slot se disponibili
    if (ts.data.next_6_hours?.details?.precipitation_amount && hour % 6 === 0) {
      byDay[dateStr].precip += ts.data.next_6_hours.details.precipitation_amount;
    }
  }

  const dates = Object.keys(byDay).sort().slice(0, 7);
  const data = {
    time:               dates,
    weather_code:       dates.map(d => metnoSymbolToWMO(byDay[d].symbols[Math.floor(byDay[d].symbols.length/2)] || '')),
    temperature_2m_min: dates.map(d => Math.min(...byDay[d].temps)),
    temperature_2m_max: dates.map(d => Math.max(...byDay[d].temps)),
    precipitation_sum:  dates.map(d => Math.round(byDay[d].precip * 10) / 10)
  };
  DAILY_CACHE[key] = { data, ts: Date.now() };
  console.log(`[MET] Daily OK: ${data.time.length} days`);
  return data;
}
...
async function fetchWeatherForPlace(placeKey) {
  ...
  // Met.no daily — 7 giorni, sostituisce i 3 di WeatherAPI
  try {
    const metDaily = await fetchMetnoDaily(p.lat, p.lon);
    vF.daily = metDaily;
    const ed = p.lifts - p.town;
    lF.daily = {
      time:               metDaily.time,
      weather_code:       metDaily.weather_code,
      temperature_2m_min: metDaily.temperature_2m_min.map(t => t - ed/100*0.6),
      temperature_2m_max: metDaily.temperature_2m_max.map(t => t - ed/100*0.6),
      precipitation_sum:  metDaily.precipitation_sum
    };
  } catch(e) {
    console.warn(`[MET] Daily failed: ${e.message} — fallback WeatherAPI 3 days`);
  }
